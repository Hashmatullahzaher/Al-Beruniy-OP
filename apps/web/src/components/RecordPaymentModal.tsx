...
export function RecordPaymentModal({
  ...
  if (!open || !customer || !unit || !contract) return null
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4" role="dialog" aria-modal="true">
      ...
      <div
        className="relative z-10 max-h-[92vh] w-full max-w-xl overflow-y-auto rounded-2xl bg-white p-6 shadow-2xl border border-slate-200 sm:p-8"
        style={{ animation: 'fade-up 0.2s cubic-bezier(0.16, 1, 0.3, 1)' }}
      >
        {/* Demo Tag Banner */}
        <div className="mb-4 rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs text-amber-900 flex items-center gap-2">
          <Info className="h-4 w-4 shrink-0 text-amber-700" />
          <span><strong>DEMO DATA SIMULATION:</strong> This modal records preview cash into local state. No live ledger or accounting posting occurs in Stage 0.</span>
        </div>
