  ...
  return (
    <>
      <motion.div
        ...
        className={`group relative overflow-hidden rounded-2xl border border-ink-100 bg-ink-50 ${className}`}
      >
        {hasImage ? (
          <img src={image} alt={`Floor plan ${label ?? ''}`} onError={() => setFailed(true)} className="h-full w-full object-contain" loading="lazy" />
        ) : (
          <div className="relative flex h-full w-full flex-col items-center justify-center gap-2 bg-gradient-to-br from-ink-900 to-brand-950 p-4 text-center text-white/80">
            ...
            <Scan className="relative h-8 w-8 text-brand-300" />
            <div className="relative font-display text-base font-bold">{label ? `Plan ${label}` : 'Floor plan'}</div>
        ...
        {(label || area) && (
          <div className="pointer-events-none absolute inset-x-0 top-0 flex items-start justify-between p-3">
            {label && <span className="rounded-lg bg-ink-950/70 px-2.5 py-1 text-xs font-bold text-white backdrop-blur">Plan {label}</span>}
            {area && <span className="inline-flex items-center gap-1 rounded-lg bg-white/85 px-2.5 py-1 text-xs font-bold text-ink-800 backdrop-blur"><Ruler className="h-3.5 w-3.5" /> {area} m²</span>}
          </div>
        )}
        ...
        <div className="absolute bottom-3 end-3 flex gap-2 opacity-0 transition group-hover:opacity-100">
          {onUpload && hasImage && (
            <button onClick={() => inputRef.current?.click()} className="grid h-9 w-9 place-items-center rounded-lg bg-white/85 text-ink-700 shadow-soft backdrop-blur transition hover:bg-white" aria-label="Replace image" title="Replace image">
              <Upload className="h-4 w-4" />
            </button>
          )}
          {enableZoom && hasImage && (
            <button onClick={() => setZoom(true)} className="grid h-9 w-9 place-items-center rounded-lg bg-white/85 text-ink-700 shadow-soft backdrop-blur transition hover:bg-white" aria-label="Enlarge" title="Enlarge">
