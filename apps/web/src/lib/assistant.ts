...
export function askAssistant(query: string, state: DemoState): AssistantAnswer {
  ...
  const mentionedCustomer = state.customers.find((c) => q.includes(c.name.split(' ')[0].toLowerCase()))
  ...
  if (q.includes('available') || q.includes('inventory') || q.includes('units') || q.includes('sold')) {
    ...
    const by = { available: 0, sold: 0, reserved: 0, under_contract: 0, on_hold: 0 } as Record<string, number>
    units.forEach((u) => { by[u.status]++ })
    return {
      ...
      highlight: [
        { label: 'Available', value: String(by.available) },
        { label: 'Committed', value: String(by.under_contract + by.sold) },
        { label: 'Reserved', value: String(by.reserved) },
      ],
    }
  }
